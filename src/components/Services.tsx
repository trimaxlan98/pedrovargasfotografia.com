...
export default function Services() {
  ...
  return (
    <section
      id="servicios"
      className="section-padding bg-[#F1ECE2]"
    ...
    >
      <div className="max-w-[1400px] mx-auto">
        ...
        {/* Grid */}
        <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-px bg-[#D8D1C5]">
          {services.map((service, i) => {
            ...
            return (
              <motion.div
                ...
                className="group relative bg-[#F7F4EE] p-8 hover:bg-[#FDFCF9] transition-colors duration-400 overflow-hidden"
              >
                {/* Ghost number */}
                <span
                  className="absolute -top-4 -right-2 font-cormorant font-bold text-[8rem] leading-none text-black/6 select-none pointer-events-none group-hover:text-black/10 transition-colors duration-500"
                >
                  {service.num}
                </span>
                ...
                {/* Icon */}
                <div className="mb-5 w-10 h-10 flex items-center justify-center border border-black/15 group-hover:border-gold/40 transition-colors duration-400">
                  <Icon className="w-4 h-4 text-black/40 group-hover:text-gold transition-colors duration-400" />
                </div>

                {/* Content */}
                <h3 className="font-cormorant text-ivory text-2xl font-light mb-3 group-hover:text-black transition-colors">
                  {service.title}
                </h3>
                <p className="font-dm text-black/55 text-sm leading-relaxed mb-6">{service.desc}</p>

                {/* Price + CTA */}
                <div className="flex items-center justify-between">
                  <span className="font-cormorant italic text-gold text-lg">{service.price}</span>
                  <ArrowRight className="w-4 h-4 text-black/25 group-hover:text-gold group-hover:translate-x-1 transition-all duration-300" />
                </div>
